...
from app.readers.db import add_to_db, delete_to_db, update_to_db
from app.readers.models import ReaderModel
from app.readers.schemas import Reader, ReaderAdd, ReaderDelete, ReaderUpdate
from app.readers.utils import (get_check_new_email, get_check_reader,
                               get_check_reader_in_db)
...
def add_to_reader(
    ...
    return {"Добавлено": {"name": data_reader.name, "email": data_reader.email}}


@router.patch("/reader/update")
def update_reader(
    data_reader: ReaderUpdate,
    user_data: UserModel = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    try:
...
def delete_to_reader(
    data_reader: ReaderDelete,
    user_data: UserModel = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    try:
    ...
    return {"Читатель удалён": {"name": data_reader.name, "email": data_reader.email}}
