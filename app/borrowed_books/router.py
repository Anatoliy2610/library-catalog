...
from app.borrowed_books.schemas import BorrowedBooks, BorrowedBooksAdd, BorrowedBooksRemove
from app.users.auth import get_db
from app.users.user import get_current_user
from app.users.models import UserModel
from app.borrowed_books.utils import (get_check_book, get_check_reader, get_check_count_book,
                                      get_check_issuance, get_check_count_book_for_reader)
...
def get_borrows(user_data: UserModel = Depends(get_current_user), db: Session = Depends(get_db)):
...
def remove_to_borrows(data_borrow: BorrowedBooksRemove, user_data: UserModel = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        get_check_book(book_id=data_borrow.book_id, db=db)
        get_check_reader(reader_id=data_borrow.reader_id, db=db)
...
def get_books(reader_id: Annotated[int, Path(title='Здесь указывается id поста', ge=1, lt=100)],
